...
class MasterStructureModel extends Model
{
  ...
  public function searchMinistry($search_key)
  {
    try {
      $builder = $this->db->table('master_org_hierarchy');
      $builder->select('ms_id, ms_name, SIMILARITY(ms_name,\'' . $search_key . '\')');
      $builder->where('(SIMILARITY(ms_name,\'' . $search_key . '\') > 0.1)', NULL, FALSE);
      $builder->distinct();
      $builder->orderBy('SIMILARITY(ms_name,\'' . $search_key . '\') desc');
